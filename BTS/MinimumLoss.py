...
class BinarySearchTree:
    ...
    def FindMax(self,CurrNode):
      ...
      return current
    def FindMaxCuNode(self,CuNode):
            FindNode = None
            if(CuNode.hasLeftChild()):
                FindNode = self.FindMax(CuNode.leftChild)
            return FindNode
...
if __name__ == '__main__':
    ...
    NumYear = int(sys.stdin.readline().rstrip().split()[0])
    LNum = [int(x) for x in sys.stdin.readline().rstrip().split()]
    mytree = BinarySearchTree()
    for ind,c in enumerate(LNum):
        mytree.put(c,ind)
    mytree.FindMinimumCos(NumYear,mytree.root)
    print(mytree.MinCos)
    pass